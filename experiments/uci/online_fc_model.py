...
sys.path.append("../../")
sys.path.append("..")
from src.models.utils import memory_picker, fixed_Z
...
def run_experiment(cfg: DictConfig):
    ...
    for train_data, test_data in zip(all_train_data, all_test_data):
        ...
        nlpd_vals, eval_vals, time_vals = call(cfg.optimize)(online_gp=online_gp, train_data=online_data, test_data=test_data,
                                                  debug=True)
        end_time = time.time()
        ...
        np.savez(os.path.join(output_dir, "training_statistics_" + str(k_fold_id) + ".npz"), nlpd=nlpd_vals,
                 eval=eval_vals, time_vals=time_vals)
